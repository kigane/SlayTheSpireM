...
namespace QFramework
{
    ...
    public abstract class Architecture<T> : IArchitecture where T : Architecture<T>, new() 
    {
        ...
        static void MakeSureArchitecture()
        {
            if (mArchitecture == null)
            {
                ...
                mArchitecture.mModels.Clear(); //? 为什么要清空。 因为只需要初始化一次。
        ...
        public void RegisterSystem<TSystem>(TSystem system) where TSystem : ISystem
        {
            ...
            mContainer.Register<TSystem>(system); // 向IOC容器注册实例

            // 在architecture的Init()方法中注册
            // 将实例添加到集合中，待architecture初始化一起初始化，或在architecture已经初始化后，直接初始化。
            if (!mInited)
            {
                mSystems.Add(system);
            }
    ...
    /// <summary>
    /// 标记Model,Utility,System等属于某个Architecture。用于解决循环依赖问题
    /// 例如：在Model中需要使用Utility，而在Architecture初始化时会调用Model的
    /// 构造方法，而Model的构造方法中又用Architecture获取了Utility，而Architecture
    /// 每次获取时都会调用MakeSureArchitecture方法，此时初始化没有完成，故会再次
    /// 调用Architecture的Init方法。产生递归调用。
    /// </summary>
    ...
    public class IOCContainer
    {
        ...
        public T Get<T>() where T : class
        {
            ...
            // out 表示传入的参数是按引用传递，调用用out定义参数的方法时，相应的参数前也要加out关键字，如下所示。

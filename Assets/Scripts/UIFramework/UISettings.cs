...
namespace deVoid.UIFramework
{
    ...
    public class UISettings : ScriptableObject
    {
        ...
        [Tooltip("The class name of Screens to register, should have prefabs of the same name in Resources/Prefabs/Screens/ folder")]
        [SerializeField] private string screenIdsClassName = null;
        ...
        public UIFrame CreateUIInstance(bool instanceAndRegisterScreens = true)
        {
            ...
            {
                var screenIds = Helper.GetAllConstFields(screenIdsClassName);
                foreach (var id in screenIds)
                {
                    GameObject screenGO = Resources.Load<GameObject>("Prefabs/UI/Screens/" + id);
                    var screenInstance = Instantiate(screenGO);
                    screenInstance.name = id;

                    if (screenInstance.TryGetComponent<IUIScreenController>(out var screenController))
                    {
                        newUI.RegisterScreen(id, screenController, screenInstance.transform);
                    ...
                    {
                        Debug.LogError("[UIConfig] Screen doesn't contain a ScreenController! Skipping " + id);
                    }
                }
        ...
        //     {
        //         if (!screensToRegister[i].TryGetComponent<IUIScreenController>(out var screenCtl))
        //         {
